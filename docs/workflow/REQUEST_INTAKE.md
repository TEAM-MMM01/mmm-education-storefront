...
The funded request path uses Formspree to create an internal request record. It
does not collect payment, approve a product for TEFA or PDSES/ClassWallet, or
accept an order. `config/request-intake.json` is intentionally disabled and has
